...
UR_APIEXPORT ur_result_t UR_APICALL urSamplerCreate(
    ...
    const ur_sampler_property_t *Props, ///< [in] specifies a list of sampler property names and their
                                        ///< corresponding values.
    ur_sampler_handle_t *Sampler ///< [out] pointer to handle of sampler object created
) {
    ...
    while (*CurProperty != 0) {
      ...
      case UR_SAMPLER_PROPERTIES_NORMALIZED_COORDS: {
        ...
        if (CurValueBool == PI_TRUE)
          ...
          zePrint("urSamplerCreate: unsupported "
      ...
      case UR_SAMPLER_PROPERTIES_ADDRESSING_MODE: {
        ...
        case UR_SAMPLER_ADDRESSING_MODE_MIRRORED_REPEAT:
          ...
          break;
        default:
          zePrint("urSamplerCreate: unsupported UR_SAMPLER_PROPERTIES_ADDRESSING_MODEE "
                  "value\n");
          zePrint("UR_SAMPLER_PROPERTIES_ADDRESSING_MODEE=%d\n", CurValueAddressingMode);
          return UR_RESULT_ERROR_INVALID_VALUE;
        }
      } break;
      ...
      case UR_SAMPLER_PROPERTIES_FILTER_MODE: {
        ...
        if (CurValueFilterMode == UR_EXT_SAMPLER_FILTER_MODE_NEAREST)
          ...
          zePrint("UR_SAMPLER_FILTER_MODE=%d\n", CurValueFilterMode);
          zePrint(
  ...
  ZE2UR_CALL(zeSamplerCreate, (Context->ZeContext,
                               Device->ZeDevice,
                               &ZeSamplerDesc, // TODO: translate properties
                               &ZeSampler));

  try {
    _ur_sampler_handle_t *UrSampler = new _ur_sampler_handle_t(ZeSampler);
...
UR_APIEXPORT ur_result_t UR_APICALL urSamplerRetain(
    ur_sampler_handle_t Sampler ///< [in] handle of the sampler object to get access
) {
...
UR_APIEXPORT ur_result_t UR_APICALL urSamplerRelease(
    ur_sampler_handle_t Sampler ///< [in] handle of the sampler object to release
) {
  if (!Sampler->RefCount.decrementAndTest())
    return UR_RESULT_SUCCESS;

  ZE2UR_CALL(zeSamplerDestroy, (Sampler->ZeSampler));
  delete Sampler;
